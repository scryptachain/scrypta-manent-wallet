<template>
  <div class="page" v-if="!isLoading">
    ...
    <div style="position:relative;">
      <h1 class="is-title is-2" style="position:absolute; top: 0; right:0; font-size:14px; text-align:right">{{ balance }}<br>{{ ticker }}</h1>
      <h1 class="is-title is-2" style="margin-top:8px; text-align:left;">
        {{ $t('dashboard.welcome_back') }}
        <div style="font-size:14px; margin-top:-15px;"><br>{{ wallet.master.substr(0,6) }}...{{ wallet.master.substr(-6) }} <button v-clipboard="wallet.master">copy</button></div>
      </h1>
    </div>
    <apexchart v-if="!isLoading && confirmed.length > 0" height="300px" width="100%" type="line" :options="options" :series="series"></apexchart>
    <div v-if="confirmed.length === 0" style="padding:50px 0; text-align:center">
      {{ $t('dashboard.no_txs') }}
    </div>
    <div v-if="unconfirmed.length > 0" style="padding:0 20px; text-align:left">
      ...
      <div v-if="confirmed.length > 0" style="padding:0 20px; text-align:left">
        ...
        >
          <div class="card-content">
            <div class="media">
              ...
              <div class="media-content" style="overflow:hidden">
                ...
                <p class="subtitle is-6" style="font-size:12px">
                  ...
                  <span v-if="tx.value > 0">
                    {{$t('dashboard.from')}}
                    <b
                    >{{ tx.from.substr(0,3) }}...{{ tx.from.substr(-3) }}</b>
                  </span>
                  <span v-if="tx.value < 0 && tx.to.length > 0">
                    {{$t('dashboard.to')}}
                    <b
                    >{{ tx.to.substr(0,3) }}...{{ tx.to.substr(-3) }}</b>
                  </span>
                  <span v-if="tx.value < 0 && tx.to.length === 0">
                    {{$t('dashboard.stored')}}
                  </span>
              ...
              <a :href="'https://explorer.scryptachain.org/transaction/' + tx.txid" target="_blank">
                <b-icon
                  icon="share"
                  style="
                    position: absolute;
        ...
        <div class="text-center">
          <b-button v-if="confirmed.length > chunkSize" @click="showMore" type="is-primary">{{ $t('dashboard.showmore') }}</b-button>
        </div>
      </div>
  </div>
</template>
